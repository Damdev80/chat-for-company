...
const MessageInput = ({ 
  ...
  const handleAudioReady = async (audioBlob) => {
    try {
      ...
      const result = await uploadAudioMessage(audioBlob, groupId, token);
      ...
      if (result.success) {
        ...
        // Refrescar mensajes o manejar el nuevo mensaje
        if (onSendMessage) {
          // Simular un evento de envío para refrescar
          onSendMessage({ preventDefault: () => {} }, null);
        }
      }
    } catch (error) {
