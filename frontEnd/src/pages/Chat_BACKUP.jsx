...
function Chat() {
  ...
  const [notification, setNotification] = useState(null);  // Estado para usuarios
  const [users, setUsers] = useState([]);
  // Estado para forzar actualización de progreso de objetivos
  const [objectiveRefreshKey, setObjectiveRefreshKey] = useState(0);

  // Obtener datos reales del usuario
  const token = localStorage.getItem("token");
  const user = localStorage.getItem("username") || "";  const userRole = localStorage.getItem("userRole") || "user"; // Leer rol del usuario
  ...
  useEffect(() => {
    ...
    socket.emit("join_group", activeGroup);    // Recibir mensajes
    ...
    socket.on("task_assigned", (data) => {
      if (data.assigned_to === user) {
        showNotification(
          ...
          `Se te ha asignado la tarea: ${data.title}`
        );
      }
    });
    ...
    socket.on("progress_update", (data) => {
      if (data.group_id === activeGroup) {
        // Force refresh of objectives and tasks data
        // This will trigger re-rendering of ObjectiveProgress components
        const refreshEvent = new CustomEvent('objectiveProgressUpdate', {
          detail: data
        });
        ...
        if (data.is_completed) {
          showNotification(
            "¡Objetivo completado!",
            `¡${data.objective_title} alcanzó el 100%! 🎉`
          );
        } else if (data.progress >= 75) {
          showNotification(
            "Progreso excelente",
            `${data.objective_title}: ${data.progress}% completado`
          );
  ...
  // Función para refrescar el progreso de objetivos cuando se completa una tarea
  const handleTaskUpdate = async () => {
    // Force re-render of ObjectiveProgressSummary by updating the refresh key
  ...
  const handleCreateGroup = async (e) => {
    ...
    try {
      ...
      showNotification(
        "Grupo creado",
        `El grupo "${currentGroupName}" ha sido creado exitosamente`
      );
    } catch (error) {
  ...
  const handleEditGroup = async (id, newName) => {
    ...
    try {
      ...
      showNotification(
        "Grupo actualizado",
        `El grupo ha sido renombrado a "${newName}"`
      );
    } catch {
      // feedback opcional
    }
  ...
  const handleDeleteGroupMessages = async () => {
    ...
    if (
      !window.confirm(`¿Eliminar todos los mensajes de "${groupNameToDelete}"?`)
    )
      return;
    try {
      ...
      showNotification(
        "Mensajes borrados",
        `Se eliminaron todos los mensajes de "${groupNameToDelete}"`
      );
    } catch {
    ...
    <div className="min-h-screen bg-[#1E1E2E] text-white flex flex-col">
      ...
      {notification && (
        <div className="fixed top-4 right-4 z-50 bg-[#2D2D3A] border border-[#3C3C4E] rounded-lg shadow-lg p-4 max-w-xs animate-slideInRight">
          <div className="flex items-start">
            ...
            <button
              className="ml-4 text-[#A0A0B0] hover:text-white"
      ...
      <div className="relative z-10 flex flex-col h-screen">
        ...
        <div className="flex-1 flex overflow-hidden">
              ...
              >
                ...
                <div className="p-4">
                  <div className="relative">
                    ...
                    <Search
                      size={16}
                      className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#A0A0B0]"
                    />
                  </div>
                </div>{" "}
                {/* Pestañas */}
                ...
                <div className="p-4 border-b border-[#3C3C4E] overflow-y-auto flex-grow">
                  ...
                  {activeTab === "contacts" && (
                    <div className="mt-6">
                      ...
                      <ul className="space-y-2">
                        {users.length === 0 ? (
                          <li className="text-[#A0A0B0] text-sm">
                            No hay usuarios
                          </li>
                        ) : (
                          users.map((u) => (
                    ...
                    <div className="mt-6">
                      <div className="text-center text-[#A0A0B0] text-sm">
                        ...
                        <p className="text-xs mt-1">
                          Selecciona un grupo para ver sus objetivos
                        </p>
                      </div>
                    </div>
                  )}
          ...
          <aside
            className="w-80 bg-[#2D2D3A] border-r border-[#3C3C4E] flex-col hidden md:flex animate-fadeInLeft min-w-[280px] max-w-[600px] relative"
            id="sidebar"
          >
            {/* Handle para redimensionar */}
            <div
              className="absolute right-0 top-0 bottom-0 w-2 bg-[#3C3C4E] hover:bg-[#4ADE80] cursor-ew-resize transition-colors z-10"
              onMouseDown={(e) => {
                e.preventDefault();
                ...
                <div className="mt-6">
                  <div className="text-center text-[#A0A0B0] text-sm bg-[#4ADE80]/5 border border-[#4ADE80]/20 rounded-lg p-4">
                    ...
                    <p className="font-medium text-white mb-1">
                      ¡Gestión de Objetivos Activada!
                    </p>
                    <p className="text-xs">
                      El área principal ahora muestra los objetivos
                    </p>
          ...
          <main className="flex-1 flex flex-col bg-[#1E1E2E] animate-fadeIn relative">
            ...
            {activeTab === "objectives" ? (
              ...
              <div className="flex-1 flex flex-col h-full">
                <div className="bg-[#2D2D3A] border-b border-[#3C3C4E] p-4 flex-shrink-0">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#A0A0B0]">
                        Gestión de objetivos y tareas del grupo
                      </p>
                    </div>                  </div>                </div>                <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                  {userRole === "admin" ? (
                    <ObjectiveManager
                      groupId={activeGroup}
                      token={token}
                ...
                {showGroupInfo && (
                  <div className="absolute inset-0 bg-[#1E1E2E] z-20 flex flex-col animate-fadeIn">
                    <div className="bg-[#2D2D3A] border-b border-[#3C3C4E] p-4 flex items-center">
                      ...
                      <h2 className="text-lg font-bold text-[#4ADE80]">
                  ...
                  {filteredMessages.length === 0 ? (
                    ...
                    <>
                      ...
                      <div className="flex justify-center">
                        <div className="bg-[#2D2D3A] text-[#A0A0B0] text-xs px-3 py-1 rounded-full">
                          Hoy
                        </div>
                      </div>
                      ...
                      {filteredMessages.map((message, index) => {
                        ...
                        const showSender =
                          index === 0 ||
                          filteredMessages[index - 1].sender_name !==
                            message.sender_name ||
                                ...
                                >
                                  ...
                                  {message.isMine &&
                                    (message.error ? (
                                      ...
                                      <Check
                                        size={14}
                                        className="ml-1 text-black text-opacity-70"
                                      />
                                    ))}
                                </div>
      ...
      {/* Modal mejorado para crear grupo */}{" "}
      {showGroupModal && (
        <div
        ...
        >
          <div
            ...
            style={{
              boxShadow:
                "0 25px 50px -12px rgba(0, 0, 0, 0.8), 0 0 0 1px rgba(74, 222, 128, 0.1)",
            }}
          >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[#A0A0B0] text-sm mt-1">
                    Organiza conversaciones por equipos y proyectos
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleCreateGroup} className="space-y-7">
              ...
              <div className="space-y-3">
                <label
                  htmlFor="group-name"
                  className="flex items-center gap-2 text-sm font-semibold text-[#A0A0B0]"
                >
                ...
                <div className="relative group">
                  ...
                  {/* Indicador de validación */}                  {groupName.trim() && (
                    <div
                      className={`absolute right-16 top-1/2 transform -translate-y-1/2 w-3 h-3 rounded-full ${
                        groupName.length <= 50 ? "bg-[#4ADE80]" : "bg-orange-400"
                      } animate-pulse`}
                    />
                  )}
                </div>
            ...
            <form
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  showNotification(
                    "Grupo actualizado",
                    `El grupo ha sido renombrado a "${editGroupName}"`
                  );
                } catch {
                  showNotification(
                    "Error",
            ...
            >
              ...
              <div>
                <label
                  ...
                  className="block text-sm font-medium text-[#A0A0B0] mb-1"
                >
